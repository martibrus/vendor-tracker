...
function Scheduling({vendors,initVid,clearInit,addSlot,rmSlot,setResp,addAv,rmAv}){
  ...
  const [na, setNa] = useState({ date: "", timeFrom: "", timeTo: "", vendorId: vendors[0]?.id || "" });

  // Sync checkbox state when vendors change
  useEffect(() => { setVisV(prev => { const m = { ...prev }; vendors.forEach(v => { if (!(v.id in m)) m[v.id] = true; }); return m; }); }, [vendors]);
  // Handle initVid from notifications
  ...
  return (
    <div style={{ display: "flex", gap: 20, alignItems: "flex-start", flexWrap: "wrap" }}>
      ...
      {/* LEFT: Vendor checkboxes */}
      <div style={{ flex: "0 0 200px", ...pnl }}>
        <h4 style={{ margin: "0 0 12px", fontSize: 14, fontWeight: 700, color: "#334155" }}>📋 Vendor visibili</h4>
        <div style={{ display: "flex", gap: 4, marginBottom: 12 }}>
          <button onClick={() => { const m = {}; vendors.forEach(v => { m[v.id] = true; }); setVisV(m); }} style={{ fontSize: 11, padding: "3px 8px", borderRadius: 6, border: "1px solid #E2E8F0", background: "white", cursor: "pointer", color: "#10B981", fontWeight: 600 }}>Tutti</button>
          <button onClick={() => { const m = {}; vendors.forEach(v => { m[v.id] = false; }); setVisV(m); }} style={{ fontSize: 11, padding: "3px 8px", borderRadius: 6, border: "1px solid #E2E8F0", background: "white", cursor: "pointer", color: "#EF4444", fontWeight: 600 }}>Nessuno</button>
        </div>
        {vendors.map(v => (
          <label key={v.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 0", cursor: "pointer", fontSize: 14, color: visV[v.id] ? "#0F172A" : "#CBD5E1", borderBottom: "1px solid #F8FAFC" }}>
            <input type="checkbox" checked={!!visV[v.id]} onChange={() => setVisV(p => ({ ...p, [v.id]: !p[v.id] }))} style={{ accentColor: "#6366F1", cursor: "pointer", width: 16, height: 16 }} />
            <div style={{ width: 24, height: 24, borderRadius: 6, background: "linear-gradient(135deg,#6366F1,#8B5CF6)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 700, fontSize: 11, flexShrink: 0 }}>{v.name.charAt(0).toUpperCase()}</div>
            <span style={{ fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{v.name}</span>
          </label>
        ))}
      </div>

      {/* RIGHT: All visible vendors stacked */}
      <div style={{ flex: "1 1 0", minWidth: 0, display: "flex", flexDirection: "column", gap: 28 }}>
        {shownVendors.length === 0 && <div style={{ textAlign: "center", padding: 40, color: "#94A3B8" }}><p style={{ fontSize: 14 }}>Seleziona almeno un vendor dalla lista a sinistra.</p></div>}
          ...
          return (
            <div key={vendor.id}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                <div style={{ width: 36, height: 36, borderRadius: 9, background: "linear-gradient(135deg,#6366F1,#8B5CF6)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 700, fontSize: 16 }}>{vendor.name.charAt(0).toUpperCase()}</div>
                <div>
                  <h3 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>{vendor.name}</h3>
                  <p style={{ margin: 0, fontSize: 12, color: "#64748B" }}>{vendor.crm || "—"} · {slots.length} slot</p>
                </div>
              </div>

              {/* Add slot form for this vendor */}
              <div style={{ ...pnl, marginBottom: 12 }}>
                <h4 style={{ margin: "0 0 10px", fontSize: 14, fontWeight: 700 }}>📅 Aggiungi slot per {vendor.name}</h4>
              ...
              {/* Slots */}
              {slots.length === 0 && <p style={{ color: "#94A3B8", fontSize: 13, textAlign: "center", padding: 16 }}>Nessuno slot proposto per questo vendor.</p>}
                ...
                {slots.map(s => {
                  ...
                  return (
                    <div key={s.id} style={{ borderRadius: 14, border: "1px solid #E2E8F0", overflow: "hidden" }}>
                      <div style={{ display: "flex", alignItems: "center", padding: "14px 20px", background: "linear-gradient(135deg,#EFF6FF,#F0F9FF)", borderBottom: "2px solid #DBEAFE", gap: 12 }}>
                        <div style={{ flex: 1 }}><div style={{ fontSize: 16, fontWeight: 700 }}>{fdt(s.date, s.time)}</div><div style={{ fontSize: 12, color: "#3B82F6", marginTop: 2, fontWeight: 600 }}>{sLabel}</div></div>
                      ...
                      <div style={{ background: "white" }}>
                        {stepParts.length > 0 && <div style={{ display: "grid", gridTemplateColumns: "120px 1fr 1fr 120px", padding: "8px 20px", borderBottom: "1px solid #F1F5F9", background: "#FAFBFC" }}>
                        ...
                        {stepParts.length === 0 && <div style={{ padding: 16, textAlign: "center", color: "#94A3B8", fontSize: 13 }}>⚠️ Nessun partecipante assegnato.</div>}
                        {stepParts.map((p, pi) => { const val = resp[p]; return (
                          <div key={p} style={{ display: "grid", gridTemplateColumns: "120px 1fr 1fr 120px", alignItems: "center", padding: "10px 20px", borderBottom: pi < stepParts.length - 1 ? "1px solid #F8FAFC" : "none", background: val === "yes" ? "#F0FDF4" : val === "no" ? "#FFF5F5" : "white" }}>
              ...
              {/* Availability for this vendor */}
              <div style={{ ...pnl, marginTop: 12 }}>
                <h4 style={{ margin: "0 0 10px", fontSize: 14, fontWeight: 700 }}>🕐 Disponibilità alternative – {vendor.name}</h4>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                  <input type="date" style={{ ...inp, flex: "1 1 130px" }} value={na.vendorId === vendor.id ? na.date : ""} onChange={e => setNa({ ...na, date: e.target.value, vendorId: vendor.id })} />
                  <input type="time" style={{ ...inp, flex: "0 1 100px" }} value={na.vendorId === vendor.id ? na.timeFrom : ""} onChange={e => setNa({ ...na, timeFrom: e.target.value, vendorId: vendor.id })} />
                  <span style={{ alignSelf: "center", color: "#64748B" }}>–</span>
                  <input type="time" style={{ ...inp, flex: "0 1 100px" }} value={na.vendorId === vendor.id ? na.timeTo : ""} onChange={e => setNa({ ...na, timeTo: e.target.value, vendorId: vendor.id })} />
                  <button style={{ padding: "10px 18px", borderRadius: 8, border: "none", background: "#10B981", color: "white", fontWeight: 600, fontSize: 13, cursor: "pointer" }} onClick={() => { if (!na.date || !na.timeFrom || !na.timeTo || na.vendorId !== vendor.id) return; addAv(vendor.id, { date: na.date, timeFrom: na.timeFrom, timeTo: na.timeTo }); setNa({ date: "", timeFrom: "", timeTo: "", vendorId: vendor.id }); }}>+ Aggiungi</button>
                </div>
                {(vendor.availability || []).length === 0 && <p style={{ color: "#94A3B8", fontSize: 13, textAlign: "center", padding: 8 }}>Nessuna disponibilità.</p>}
                {(vendor.availability || []).map(a => <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: "#F0FDF4", borderRadius: 8, borderLeft: "3px solid #10B981", marginBottom: 6 }}>
              ...
              <div style={{ height: 2, background: "linear-gradient(90deg, #E2E8F0, transparent)", margin: "12px 0 0" }} />
            </div>
          );
        })}
      </div>
