...
// ===================== SCHEDULING =====================
function Scheduling({vendors,initVid,clearInit,addSlot,rmSlot,setResp,addAv,rmAv}){const [sel,setSel]=useState(initVid||vendors[0]?.id||"");const [ns,setNs]=useState({date:"",time:"",step:"step1"});const [na,setNa]=useState({date:"",timeFrom:"",timeTo:""});useEffect(()=>{if(initVid){setSel(initVid);clearInit();}},[initVid]);const vendor=vendors.find(v=>v.id===sel);const slots=fix(vendor?.slots);if(!vendors.length)return<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"50vh",textAlign:"center"}}><div style={{fontSize:52,marginBottom:16}}>📅</div><h2 style={{margin:0,fontSize:22,fontWeight:700}}>Nessun vendor</h2></div>;return(<div style={{display:"flex",flexDirection:"column",gap:24}}><div style={{display:"flex",alignItems:"center",gap:12}}><span style={{fontSize:14,fontWeight:600,color:"#64748B"}}>Vendor:</span><select style={{...inp,width:"auto",minWidth:220}} value={sel} onChange={e=>setSel(e.target.value)}>{vendors.map(v=><option key={v.id} value={v.id}>{v.name}</option>)}</select></div>{vendor&&<><div style={pnl}><h3 style={{margin:"0 0 16px",fontSize:16,fontWeight:700}}>📅 Slot proposti dal Vendor</h3><p style={{fontSize:13,color:"#64748B",margin:"0 0 16px"}}>Solo i partecipanti assegnati allo step vedranno gli slot.</p><div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:24,padding:16,background:"#F8FAFC",borderRadius:10,border:"1px dashed #CBD5E1"}}><input type="date" style={{...inp,flex:"1 1 130px"}} value={ns.date} onChange={e=>setNs({...ns,date:e.target.value})}/><input type="time" style={{...inp,flex:"1 1 100px"}} value={ns.time} onChange={e=>setNs({...ns,time:e.target.value})}/><select style={{...inp,flex:"1 1 120px"}} value={ns.step} onChange={e=>setNs({...ns,step:e.target.value})}>{STEPS.map(s=><option key={s.key} value={s.key}>{s.short}</option>)}</select><button style={{padding:"10px 18px",borderRadius:8,border:"none",background:"#3B82F6",color:"white",fontWeight:600,fontSize:13,cursor:"pointer"}} onClick={()=>{if(!ns.date||!ns.time)return;addSlot(sel,ns);setNs({date:"",time:"",step:"step1"})}}>+ Aggiungi</button></div>{slots.length===0&&<p style={{color:"#94A3B8",fontSize:14,textAlign:"center",padding:32}}>Nessuno slot.</p>}<div style={{display:"flex",flexDirection:"column",gap:20}}>{slots.map(s=>{const sLabel=STEPS.find(st=>st.key===s.step)?.label||s.step;const resp=s.responses||{};const stepParts=(vendor[s.step]?.participants?.length>0)?vendor[s.step].participants:[];const yC=stepParts.filter(p=>resp[p]==="yes").length,nC=stepParts.filter(p=>resp[p]==="no").length,pC=stepParts.filter(p=>!resp[p]).length;return<div key={s.id} style={{borderRadius:14,border:"1px solid #E2E8F0",overflow:"hidden"}}><div style={{display:"flex",alignItems:"center",padding:"16px 20px",background:"linear-gradient(135deg,#EFF6FF,#F0F9FF)",borderBottom:"2px solid #DBEAFE",gap:12}}><div style={{flex:1}}><div style={{fontSize:18,fontWeight:700}}>{fdt(s.date,s.time)}</div><div style={{fontSize:13,color:"#3B82F6",marginTop:2,fontWeight:600}}>{sLabel}</div></div><div style={{display:"flex",gap:8}}><span style={{background:"#D1FAE5",color:"#065F46",padding:"4px 12px",borderRadius:100,fontSize:13,fontWeight:700}}>✓ {yC}</span><span style={{background:"#FEE2E2",color:"#991B1B",padding:"4px 12px",borderRadius:100,fontSize:13,fontWeight:700}}>✗ {nC}</span><span style={{background:"#FEF3C7",color:"#92400E",padding:"4px 12px",borderRadius:100,fontSize:13,fontWeight:700}}>⏳ {pC}</span></div><button onClick={()=>rmSlot(vendor.id,s.id)} style={{background:"none",border:"none",color:"#94A3B8",cursor:"pointer",fontSize:20}}>✕</button></div><div style={{background:"white"}}>{stepParts.length>0&&<div style={{display:"grid",gridTemplateColumns:"120px 1fr 1fr 120px",padding:"10px 20px",borderBottom:"1px solid #F1F5F9",background:"#FAFBFC"}}><span style={{fontSize:11,fontWeight:700,color:"#94A3B8",textTransform:"uppercase"}}>Chi</span><span style={{fontSize:11,fontWeight:700,color:"#94A3B8",textTransform:"uppercase",textAlign:"center"}}>Disponibile</span><span style={{fontSize:11,fontWeight:700,color:"#94A3B8",textTransform:"uppercase",textAlign:"center"}}>Non disp.</span><span style={{fontSize:11,fontWeight:700,color:"#94A3B8",textTransform:"uppercase",textAlign:"center"}}>Stato</span></div>}{stepParts.length===0&&<div style={{padding:20,textAlign:"center",color:"#94A3B8",fontSize:13}}>⚠️ Nessun partecipante assegnato.</div>}{stepParts.map((p,pi)=>{const val=resp[p];return<div key={p} style={{display:"grid",gridTemplateColumns:"120px 1fr 1fr 120px",alignItems:"center",padding:"12px 20px",borderBottom:pi<stepParts.length-1?"1px solid #F8FAFC":"none",background:val==="yes"?"#F0FDF4":val==="no"?"#FFF5F5":"white"}}><span style={{fontSize:13,fontWeight:700,padding:"5px 12px",borderRadius:100,background:PC[p].bg,color:PC[p].fg,justifySelf:"start"}}>{p}</span><div style={{display:"flex",justifyContent:"center"}}><button onClick={()=>setResp(vendor.id,s.id,p,"yes")} style={{padding:"10px 24px",borderRadius:10,fontSize:14,fontWeight:700,cursor:"pointer",border:val==="yes"?"2px solid #059669":"1.5px solid #E2E8F0",background:val==="yes"?"#D1FAE5":"white",color:val==="yes"?"#059669":"#94A3B8"}}>✓ Sì</button></div><div style={{display:"flex",justifyContent:"center"}}><button onClick={()=>setResp(vendor.id,s.id,p,"no")} style={{padding:"10px 24px",borderRadius:10,fontSize:14,fontWeight:700,cursor:"pointer",border:val==="no"?"2px solid #DC2626":"1.5px solid #E2E8F0",background:val==="no"?"#FEE2E2":"white",color:val==="no"?"#DC2626":"#94A3B8"}}>✗ No</button></div><div style={{textAlign:"center"}}>{!val&&<span style={{fontSize:12,color:"#F59E0B",fontWeight:700,animation:"pulse 2s infinite"}}>⏳</span>}{val==="yes"&&<span style={{fontSize:12,color:"#059669",fontWeight:700}}>✓</span>}{val==="no"&&<span style={{fontSize:12,color:"#DC2626",fontWeight:700}}>✗</span>}</div></div>})}</div></div>})}</div></div><div style={pnl}><h3 style={{margin:"0 0 16px",fontSize:16,fontWeight:700}}>🕐 Disponibilità alternative</h3><div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:16,padding:16,background:"#F0FDF4",borderRadius:10,border:"1px dashed #86EFAC"}}><input type="date" style={{...inp,flex:"1 1 130px"}} value={na.date} onChange={e=>setNa({...na,date:e.target.value})}/><input type="time" style={{...inp,flex:"0 1 100px"}} value={na.timeFrom} onChange={e=>setNa({...na,timeFrom:e.target.value})}/><span style={{alignSelf:"center",color:"#64748B"}}>–</span><input type="time" style={{...inp,flex:"0 1 100px"}} value={na.timeTo} onChange={e=>setNa({...na,timeTo:e.target.value})}/><button style={{padding:"10px 18px",borderRadius:8,border:"none",background:"#10B981",color:"white",fontWeight:600,fontSize:13,cursor:"pointer"}} onClick={()=>{if(!na.date||!na.timeFrom||!na.timeTo)return;addAv(sel,na);setNa({date:"",timeFrom:"",timeTo:""})}}>+ Aggiungi</button></div>{(vendor.availability||[]).length===0&&<p style={{color:"#94A3B8",fontSize:14,textAlign:"center",padding:20}}>Nessuna disponibilità.</p>}{(vendor.availability||[]).map(a=><div key={a.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",background:"#F0FDF4",borderRadius:8,borderLeft:"3px solid #10B981",marginBottom:6}}><span style={{fontWeight:600,fontSize:14}}>{new Date(a.date+"T00:00").toLocaleDateString("it-IT",{weekday:"short",day:"numeric",month:"short"})}</span><span style={{color:"#64748B",fontSize:13}}>{a.timeFrom} – {a.timeTo}</span><div style={{flex:1}}/><button onClick={()=>rmAv(vendor.id,a.id)} style={{background:"none",border:"none",color:"#94A3B8",cursor:"pointer",fontSize:14}}>✕</button></div>)}</div></>}</div>);}
