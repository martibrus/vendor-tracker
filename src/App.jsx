...
import { subscribeVendors, saveVendor, removeVendor, subscribeGantt, saveGanttTask, removeGanttTask, subscribeSettings, saveSettings } from "./firebase.js";
...
const GANTT_COLORS = ["#3B82F6", "#10B981", "#F59E0B", "#EF4444", "#8B5CF6", "#EC4899", "#06B6D4", "#F97316"];
const fix = (s) => (s || []).map(x => ({ ...x, responses: x.responses || {} }));
function makeEmpty() {
  return { id: gid(), name: "", crm: "", step1: { status: "Da Programmare", participants: [], notes: "" }, step2: { status: "Da Programmare", participants: [], notes: "" }, step3: { status: "Da Programmare", participants: [], notes: "" }, step4: { status: "Da Programmare", participants: [], notes: "" }, slots: [], availability: [] };
}
...
export default function App() {
  const [currentUser, setCurrentUser] = useState(() => {
    try { return localStorage.getItem("vt_user") || null; } catch { return null; }
  });
  ...
  function selectUser(u) {
    setCurrentUser(u);
    try { localStorage.setItem("vt_user", u); } catch {}
  }
  ...
  useEffect(() => {
    if (visibleTabs.length > 0 && !visibleTabs.find(t => t.key === view)) {
      setView(visibleTabs[0].key);
    }
  }, [tabVis, view]);

  async function sv(vendor) { try { await saveVendor(vendor); } catch (e) { alert("Errore: " + e.message); } }
  async function doSave(v) { await sv(v); setFormVendor(null); }
  async function doDel(id) { if (!confirm("Eliminare?")) return; try { await removeVendor(id); } catch (e) { alert("Errore: " + e.message); } setFormVendor(null); }
  async function addSlot(vid, sl) { const v = vendors.find(x => x.id === vid); if (!v) return; await sv({ ...v, slots: [...fix(v.slots), { ...sl, id: gid(), responses: {} }] }); }
  async function rmSlot(vid, sid) { const v = vendors.find(x => x.id === vid); if (!v) return; await sv({ ...v, slots: fix(v.slots).filter(s => s.id !== sid) }); }
  async function setResp(vid, sid, who, val) { const v = vendors.find(x => x.id === vid); if (!v) return; await sv({ ...v, slots: fix(v.slots).map(s => { if (s.id !== sid) return s; const r = { ...s.responses }; if (r[who] === val) delete r[who]; else r[who] = val; return { ...s, responses: r }; }) }); }
  async function addAv(vid, a) { const v = vendors.find(x => x.id === vid); if (!v) return; await sv({ ...v, availability: [...(v.availability || []), { ...a, id: gid() }] }); }
  async function rmAv(vid, aid) { const v = vendors.find(x => x.id === vid); if (!v) return; await sv({ ...v, availability: (v.availability || []).filter(a => a.id !== aid) }); }
  ...
  if (!currentUser) return (
    <div style={{ minHeight: "100vh", background: "#0F172A", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Outfit',sans-serif" }}>
      ...
      <div style={{ background: "white", borderRadius: 20, padding: 40, maxWidth: 420, width: "100%", textAlign: "center" }}>
        ...
        <p style={{ margin: "0 0 28px", color: "#64748B", fontSize: 15 }}>Chi sei? Seleziona il tuo profilo per continuare.</p>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {PARTS.map(p => (
            <button key={p} onClick={() => selectUser(p)} style={{ padding: "14px 20px", borderRadius: 12, border: "2px solid " + PC[p].fg + "22", background: PC[p].bg, color: PC[p].fg, fontSize: 16, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, transition: "all 0.15s" }}>
              {p} {p === ADMIN && <span style={{ fontSize: 11, background: PC[p].fg, color: "white", padding: "2px 8px", borderRadius: 100 }}>Admin</span>}
            </button>
          ))}
        </div>
      </div>
  ...
  if (loading) return <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100vh", fontFamily: "'Outfit',sans-serif" }}><style>{css}</style><div style={{ width: 32, height: 32, border: "3px solid #E2E8F0", borderTopColor: "#6366F1", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} /><p style={{ color: "#94A3B8", marginTop: 12 }}>Caricamento...</p></div>;
    ...
    <div style={{ minHeight: "100vh", background: "#F1F5F9", fontFamily: "'Outfit',sans-serif", color: "#0F172A" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 28px", background: "#0F172A", flexWrap: "wrap", gap: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div><h1 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "#F8FAFC" }}>Vendor Tracker</h1></div>
        </div>

        {/* TABS */}
        <div style={{ display: "flex", gap: 4, background: "#1E293B", borderRadius: 10, padding: 4 }}>
          {visibleTabs.map(t => (
            <button key={t.key} onClick={() => setView(t.key)} style={{ padding: "7px 14px", borderRadius: 8, border: "none", background: view === t.key ? "#334155" : "transparent", color: view === t.key ? "#F8FAFC" : "#94A3B8", cursor: "pointer", fontSize: 13, fontWeight: 500, position: "relative", opacity: (!tabVis[t.key] && isAdmin) ? 0.4 : 1 }}>
              {t.label}
              {t.key === "vendors" && notifs.length > 0 && <span style={{ position: "absolute", top: 1, right: 3, background: "#EF4444", color: "white", fontSize: 9, fontWeight: 700, borderRadius: 100, padding: "1px 4px" }}>{notifs.length}</span>}
              {!tabVis[t.key] && isAdmin && <span style={{ fontSize: 9, color: "#EF4444", marginLeft: 4 }}>nascosto</span>}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {/* User badge */}
          <span style={{ ...PC[currentUser] ? { background: PC[currentUser].bg, color: PC[currentUser].fg } : {}, padding: "5px 12px", borderRadius: 100, fontSize: 13, fontWeight: 700, cursor: "pointer" }} onClick={() => { if (confirm("Vuoi cambiare utente?")) { try { localStorage.removeItem("vt_user"); } catch {} setCurrentUser(null); } }}>{currentUser}</span>

          {/* Admin gear */}
          {isAdmin && <button onClick={() => setShowAdmin(true)} style={{ background: "none", border: "none", color: "#94A3B8", cursor: "pointer", fontSize: 18, padding: 4 }} title="Impostazioni Admin">⚙️</button>}
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 28px" }}>
        ...
        {view === "timeline" && <GanttTimeline tasks={ganttTasks} vendors={vendors} />}
      ...
      {/* VENDOR FORM MODAL */}
      {formVendor && <Modal onClose={() => setFormVendor(null)}>
        <VendorForm vendor={formVendor} onChange={setFormVendor} onSave={() => { if (!formVendor.name) { alert("Inserisci il nome"); return; } doSave(formVendor); }} onDel={vendors.find(x => x.id === formVendor.id) ? () => doDel(formVendor.id) : null} isNew={!vendors.find(x => x.id === formVendor.id)} />
      </Modal>}

      {/* ADMIN MODAL */}
      {showAdmin && isAdmin && <Modal onClose={() => setShowAdmin(false)}>
        <AdminPanel tabVis={tabVis} onSave={async (tv) => { await saveSettings(tv); setShowAdmin(false); }} />
      </Modal>}
    </div>
  );
}
...
// ===================== ADMIN PANEL =====================
function AdminPanel({ tabVis, onSave }) {
  const [local, setLocal] = useState({ ...tabVis });
  function toggle(key) { setLocal({ ...local, [key]: !local[key] }); }
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700 }}>⚙️ Gestione Tab</h2>
      <p style={{ margin: 0, color: "#64748B", fontSize: 14 }}>Scegli quali tab rendere visibili agli altri utenti. Solo tu (BRM) puoi gestire questa impostazione.</p>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {ALL_TABS.map(t => (
          <div key={t.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", background: local[t.key] ? "#F0FDF4" : "#FFF5F5", borderRadius: 10, border: "1px solid " + (local[t.key] ? "#A7F3D0" : "#FECACA"), transition: "all 0.2s" }}>
            <span style={{ fontSize: 15, fontWeight: 600 }}>{t.label}</span>
            <button onClick={() => toggle(t.key)} style={{ padding: "8px 20px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 700, background: local[t.key] ? "#D1FAE5" : "#FEE2E2", color: local[t.key] ? "#059669" : "#DC2626" }}>
              {local[t.key] ? "✓ Visibile" : "✗ Nascosto"}
            </button>
          </div>
        ))}
      </div>
      <button onClick={() => onSave(local)} style={{ padding: "12px 24px", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#3B82F6,#6366F1)", color: "white", fontWeight: 700, fontSize: 15, cursor: "pointer", marginTop: 8 }}>Salva impostazioni</button>
    </div>
  );
}
...
// ===================== GANTT TIMELINE =====================
function GanttTimeline({ tasks, vendors }) {
  const [form, setForm] = useState({ name: "", vendor: "", start: "", end: "", color: GANTT_COLORS[0], assignee: "" });
  ...
  const startMs = new Date(minDate).getTime() - 86400000;
  const endMs = new Date(maxDate).getTime() + 86400000 * 2;
  ...
  function dayLabels() {
    ...
    for (let i = 0; i < totalDays; i++) {
      ...
      labels.push({ date: d, label: d.toLocaleDateString("it-IT", { day: "numeric", month: "short" }), isWeekend: d.getDay() === 0 || d.getDay() === 6 });
    }
    return labels;
  }
  ...
  function barStyle(t) {
    ...
    const left = ((s - startMs) / (endMs - startMs)) * 100;
    const width = Math.max(((e - s) / (endMs - startMs)) * 100, 1);
    return { left: left + "%", width: width + "%" };
  }

  async function handleSave() {
    if (!form.name || !form.start || !form.end) { alert("Compila nome, data inizio e data fine"); return; }
    const task = { ...form, id: editId || gid() };
    try { await saveGanttTask(task); } catch (e) { alert("Errore: " + e.message); }
    setForm({ name: "", vendor: "", start: "", end: "", color: GANTT_COLORS[0], assignee: "" });
    setEditId(null);
  }

  async function handleDelete(id) {
    if (!confirm("Eliminare questa attività?")) return;
    try { await removeGanttTask(id); } catch (e) { alert("Errore: " + e.message); }
  }

  function startEdit(t) {
    setForm({ name: t.name, vendor: t.vendor || "", start: t.start, end: t.end, color: t.color || GANTT_COLORS[0], assignee: t.assignee || "" });
    setEditId(t.id);
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={pnl}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
          ...
          <select style={{ ...inp, flex: "1 1 140px" }} value={form.vendor} onChange={e => setForm({ ...form, vendor: e.target.value })}>
            <option value="">— Vendor —</option>
            {vendors.map(v => <option key={v.id} value={v.name}>{v.name}</option>)}
          </select>
          <select style={{ ...inp, flex: "1 1 100px" }} value={form.assignee} onChange={e => setForm({ ...form, assignee: e.target.value })}>
            <option value="">— Assegnato —</option>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            <span style={{ fontSize: 11, color: "#94A3B8", fontWeight: 600 }}>COLORE</span>
            <div style={{ display: "flex", gap: 4 }}>{GANTT_COLORS.map(c => <div key={c} onClick={() => setForm({ ...form, color: c })} style={{ width: 28, height: 28, borderRadius: 6, background: c, cursor: "pointer", border: form.color === c ? "3px solid #0F172A" : "2px solid transparent" }} />)}</div>
          </div>
          <div style={{ flex: 1 }} />
          {editId && <button onClick={() => { setForm({ name: "", vendor: "", start: "", end: "", color: GANTT_COLORS[0], assignee: "" }); setEditId(null); }} style={{ padding: "10px 18px", borderRadius: 8, border: "1px solid #E2E8F0", background: "white", color: "#64748B", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>Annulla</button>}
          <button onClick={handleSave} style={{ padding: "10px 18px", borderRadius: 8, border: "none", background: "#3B82F6", color: "white", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>{editId ? "Salva modifiche" : "+ Aggiungi"}</button>
        </div>
      </div>
      ...
      {sorted.length === 0 ? (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "30vh", textAlign: "center" }}>
          ...
          <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700 }}>Nessuna attività</h2>
          <p style={{ color: "#64748B", fontSize: 14, marginTop: 8 }}>Usa il form sopra per aggiungere la prima attività al Gantt.</p>
        </div>
      ) : (
        <div style={{ ...pnl, padding: 0, overflow: "hidden" }}>
          <div style={{ padding: "16px 20px", borderBottom: "1px solid #E2E8F0" }}>
            <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>📐 Timeline</h3>
          </div>
            ...
            <div style={{ minWidth: Math.max(totalDays * 40, 600) }}>
              ...
              <div style={{ display: "flex", borderBottom: "1px solid #E2E8F0", position: "sticky", top: 0, background: "#FAFBFC", zIndex: 2 }}>
                ...
                <div style={{ flex: 1, display: "flex", position: "relative" }}>
                  {days.map((d, i) => (
                    <div key={i} style={{ flex: "1 0 0", padding: "6px 2px", fontSize: 10, fontWeight: 600, color: d.isWeekend ? "#CBD5E1" : "#94A3B8", textAlign: "center", borderRight: "1px solid #F1F5F9", background: d.isWeekend ? "#F8FAFC" : "transparent" }}>
                      {d.label}
                    </div>
                  ))}
                </div>
              ...
              {sorted.map(t => {
                const bs = barStyle(t);
                return (
                  <div key={t.id} style={{ display: "flex", borderBottom: "1px solid #F8FAFC", minHeight: 44 }}>
                    <div style={{ width: 220, flexShrink: 0, padding: "8px 16px", display: "flex", alignItems: "center", gap: 8, borderRight: "1px solid #E2E8F0" }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11, color: "#94A3B8" }}>
                          ...
                          {t.vendor && t.assignee && <span> · </span>}
                          {t.assignee && <span style={{ color: PC[t.assignee]?.fg || "#64748B" }}>{t.assignee}</span>}
                        </div>
                      </div>
                      <button onClick={() => startEdit(t)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 14, padding: 2 }}>✏️</button>
                      <button onClick={() => handleDelete(t.id)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 14, padding: 2 }}>🗑️</button>
                    </div>
                    <div style={{ flex: 1, position: "relative", display: "flex", alignItems: "center" }}>
                      {/* Weekend backgrounds */}
                      {days.map((d, i) => d.isWeekend && <div key={i} style={{ position: "absolute", left: (i / totalDays * 100) + "%", width: (100 / totalDays) + "%", top: 0, bottom: 0, background: "#F8FAFC" }} />)}
                      {/* Bar */}
                      <div style={{ position: "absolute", ...bs, height: 28, borderRadius: 6, background: t.color || "#3B82F6", opacity: 0.85, display: "flex", alignItems: "center", paddingLeft: 8, overflow: "hidden" }}>
                        <span style={{ fontSize: 11, fontWeight: 600, color: "white", whiteSpace: "nowrap", textShadow: "0 1px 2px rgba(0,0,0,0.3)" }}>{t.name}</span>
                      </div>
                    </div>
                  </div>
                );
...
// ===================== SCHEDULING =====================
function Scheduling({ vendors, initVid, clearInit, addSlot, rmSlot, setResp, addAv, rmAv, currentUser }) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      {vendor && <>
        <div style={pnl}>
          ...
          <p style={{ fontSize: 13, color: "#64748B", margin: "0 0 16px" }}>Inserisci le date proposte. Solo i partecipanti assegnati allo step vedranno gli slot.</p>
            ...
            {slots.map(s => {
              ...
              return <div key={s.id} style={{ borderRadius: 14, border: "1px solid #E2E8F0", overflow: "hidden" }}>
                ...
                <div style={{ background: "white" }}>
                  {stepParts.length > 0 && <div style={{ display: "grid", gridTemplateColumns: "120px 1fr 1fr 120px", padding: "10px 20px", borderBottom: "1px solid #F1F5F9", background: "#FAFBFC" }}>
                  ...
                  {stepParts.length === 0 && <div style={{ padding: 20, textAlign: "center", color: "#94A3B8", fontSize: 13 }}>⚠️ Nessun partecipante assegnato. Vai in <strong>Vendor</strong> → modifica → assegna partecipanti allo step.</div>}
                  {stepParts.map((p, pi) => {
                    const val = resp[p];
                    return <div key={p} style={{ display: "grid", gridTemplateColumns: "120px 1fr 1fr 120px", alignItems: "center", padding: "12px 20px", borderBottom: pi < stepParts.length - 1 ? "1px solid #F8FAFC" : "none", background: val === "yes" ? "#F0FDF4" : val === "no" ? "#FFF5F5" : "white" }}>
        ...
        <div style={pnl}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16, padding: 16, background: "#F0FDF4", borderRadius: 10, border: "1px dashed #86EFAC" }}>
            <input type="date" style={{ ...inp, flex: "1 1 130px" }} value={na.date} onChange={e => setNa({ ...na, date: e.target.value })} />
            <input type="time" style={{ ...inp, flex: "0 1 100px" }} value={na.timeFrom} onChange={e => setNa({ ...na, timeFrom: e.target.value })} />
            <span style={{ alignSelf: "center", color: "#64748B" }}>–</span>
            <input type="time" style={{ ...inp, flex: "0 1 100px" }} value={na.timeTo} onChange={e => setNa({ ...na, timeTo: e.target.value })} />
          ...
          {(vendor.availability || []).map(a => <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "#F0FDF4", borderRadius: 8, borderLeft: "3px solid #10B981", marginBottom: 6 }}>
            ...
            <span style={{ color: "#64748B", fontSize: 13 }}>{a.timeFrom} – {a.timeTo}</span><div style={{ flex: 1 }} />
...
function Modal({ children, onClose }) {
  return <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(4px)" }} onClick={onClose}>
