...
const ALL_TABS = [
  ...
  { key: "calendar", label: "🗓️ Calendario" },
...
export default function App(){
  ...
  useEffect(()=>{
    try{
      ...
      const u3=subscribeSettings(d=>setTabVis(prev=>({...prev,...d})));
  ...
  async function doDel(id){if(!confirm("Eliminare?"))return;const v=vendors.find(x=>x.id===id);try{await removeVendor(id)}catch(e){alert("Errore: "+e.message)} logActivity(currentUser,"vendor_eliminato",currentUser+" ha eliminato il vendor "+(v?.name||id));setFormVendor(null);}
  async function addSlot(vid,sl){const v=vendors.find(x=>x.id===vid);if(!v)return;await sv({...v,slots:[...fix(v.slots),{...sl,id:gid(),responses:{}}]});const sL=STEPS.find(s=>s.key===sl.step)?.label||sl.step;logActivity(currentUser,"slot_aggiunto",currentUser+" ha aggiunto slot per "+v.name+" – "+sL+" ("+sl.date+" "+sl.time+")");}
  async function rmSlot(vid,sid){const v=vendors.find(x=>x.id===vid);if(!v)return;await sv({...v,slots:fix(v.slots).filter(s=>s.id!==sid)});logActivity(currentUser,"slot_rimosso",currentUser+" ha rimosso uno slot per "+v.name);}
  async function setResp(vid,sid,who,val){const v=vendors.find(x=>x.id===vid);if(!v)return;const slot=fix(v.slots).find(s=>s.id===sid);const prev=slot?.responses?.[who];const nv=prev===val?undefined:val;await sv({...v,slots:fix(v.slots).map(s=>{if(s.id!==sid)return s;const r={...s.responses};if(r[who]===val)delete r[who];else r[who]=val;return{...s,responses:r};})});if(slot){const sL=STEPS.find(st=>st.key===slot.step)?.label||slot.step;const pr=nv==="yes"?"disponibile":nv==="no"?"non disponibile":"rimosso preferenza";logActivity(who,"preferenza",who+" ha espresso preferenza per call con "+v.name+" – "+sL+": "+pr);}}
  ...
  if(!currentUser)return(
    <div style={{minHeight:"100vh",background:"#0F172A",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Outfit',sans-serif"}}><style>{css}</style>
  ...
  return(
    <div style={{minHeight:"100vh",background:"#F1F5F9",fontFamily:"'Outfit',sans-serif",color:"#0F172A"}}><style>{css}</style>
      {/* TOP BAR */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 28px",background:"#0F172A",flexWrap:"wrap",gap:10}}>
        ...
        <div style={{display:"flex",gap:4,background:"#1E293B",borderRadius:10,padding:4}}>
          {visibleTabs.map(t=><button key={t.key} onClick={()=>setView(t.key)} style={{padding:"7px 14px",borderRadius:8,border:"none",background:view===t.key?"#334155":"transparent",color:view===t.key?"#F8FAFC":"#94A3B8",cursor:"pointer",fontSize:13,fontWeight:500,position:"relative",opacity:(!tabVis[t.key]&&isAdmin)?.4:1}}>
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:"24px 28px"}}>
        ...
        {view==="calendar"&&<CalendarView vendors={vendors} ganttTasks={ganttTasks}/>}
...
// ===================== CALENDAR VIEW =====================
function CalendarView({ vendors, ganttTasks }) {
  ...
  // Build events from vendor slots + gantt tasks
    ...
    vendors.forEach(v => {
      fix(v.slots).forEach(s => {
        ...
        ev.push({
          ...
          bg: step?.bg || "#DBEAFE",
          time: s.time,
          status: v[s.step]?.status || "",
          participants: v[s.step]?.participants || [],
        });
      });
    });
    // Gantt tasks — add events for each day in range
    ganttTasks.forEach(t => {
      ...
      for (let d = new Date(s); d <= e; d.setDate(d.getDate() + 1)) {
        ev.push({
          ...
          bg: t.color ? t.color + "22" : "#EEF2FF",
          assignee: t.assignee || "",
          isStart: d.toISOString().slice(0, 10) === t.start,
          isEnd: d.toISOString().slice(0, 10) === t.end,
        });
      }
    });
    return ev;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={pnl}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>🗓️ Calendario attività</h3>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            {STEPS.map(s => (
              <div key={s.key} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <div style={{ width: 12, height: 12, borderRadius: 3, background: s.color }} />
                <span style={{ fontSize: 12, color: "#64748B", fontWeight: 600 }}>{s.short}</span>
              </div>
            ))}
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <div style={{ width: 12, height: 12, borderRadius: 3, background: "#6366F1" }} />
              <span style={{ fontSize: 12, color: "#64748B", fontWeight: 600 }}>Gantt</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 20, alignItems: "flex-start", flexWrap: "wrap" }}>
        ...
        <div style={{ ...pnl, flex: "3 1 500px", padding: 0, overflow: "hidden" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", borderBottom: "1px solid #E2E8F0" }}>
            {DAYS_IT.map((d, i) => (
              <div key={d} style={{ padding: "10px 4px", textAlign: "center", fontSize: 12, fontWeight: 700, color: i >= 5 ? "#CBD5E1" : "#64748B", background: "#FAFBFC" }}>{d}</div>
            ))}
          </div>
          ...
          {Array.from({ length: weeks }).map((_, w) => (
            <div key={w} style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)" }}>
              {Array.from({ length: 7 }).map((_, d) => {
                ...
                return (
                  <div key={d}
                    onClick={() => isValid && setSelectedDay(isSel ? null : dayNum)}
                    style={{
                      minHeight: 80, padding: "4px 6px", borderRight: "1px solid #F1F5F9", borderBottom: "1px solid #F1F5F9",
                      background: isSel ? "#EFF6FF" : isWeekend ? "#FAFBFC" : "white",
                      ...
                      <>
                        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 2 }}>
                          <span style={{
                            ...
                            color: isToday ? "white" : isWeekend ? "#CBD5E1" : "#334155",
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                          {dayEvents.slice(0, 3).map((ev, i) => (
                            <div key={i} style={{
                              ...
                              background: ev.bg, color: ev.color, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                              borderLeft: "3px solid " + ev.color,
                            }}>
                              {ev.type === "slot" ? ev.vendor : ev.label}
                            </div>
                          ))}
                          {dayEvents.length > 3 && (
                            <span style={{ fontSize: 10, color: "#94A3B8", fontWeight: 600, paddingLeft: 4 }}>+{dayEvents.length - 3} altro</span>
                          )}
                        </div>
                      </>
                    )}
        ...
        {/* Day detail sidebar */}
        <div style={{ ...pnl, flex: "1 1 280px", minWidth: 280 }}>
          {selectedDay ? (
            <>
              <h3 style={{ margin: "0 0 16px", fontSize: 16, fontWeight: 700 }}>
                {selectedDay} {MONTHS_IT[month.month]} {month.year}
              </h3>
              {selectedEvents.length === 0 && <p style={{ color: "#94A3B8", fontSize: 14, textAlign: "center", padding: 20 }}>Nessuna attività in questa data.</p>}
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {selectedEvents.map((ev, i) => (
                  <div key={i} style={{ padding: "12px 14px", borderRadius: 10, background: ev.bg, borderLeft: "4px solid " + ev.color }}>
                    {ev.type === "slot" ? (
                      <>
                        ...
                        <div style={{ fontSize: 12, color: "#64748B", marginTop: 4 }}>
                          Stato: <span style={{ fontWeight: 600 }}>{ev.status}</span>
                        </div>
                        {ev.participants.length > 0 && (
                          <div style={{ display: "flex", gap: 4, marginTop: 6, flexWrap: "wrap" }}>
                            {ev.participants.map(p => <span key={p} style={{ fontSize: 10, fontWeight: 700, padding: "2px 6px", borderRadius: 100, background: PC[p]?.bg || "#F1F5F9", color: PC[p]?.fg || "#64748B" }}>{p}</span>)}
                          </div>
                        )}
                      </>
                    ) : (
                      <>
                        ...
                        {ev.assignee && <div style={{ fontSize: 12, marginTop: 4 }}><span style={{ fontWeight: 600, color: PC[ev.assignee]?.fg || "#64748B" }}>{ev.assignee}</span></div>}
                        <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 4 }}>{ev.isStart && "▶ Inizio"}{ev.isEnd && " ■ Fine"}{!ev.isStart && !ev.isEnd && "━ In corso"}</div>
                      </>
                    )}
                  </div>
                ))}
            ...
            <div style={{ textAlign: "center", padding: 20, color: "#94A3B8" }}>
              ...
              <p style={{ fontSize: 14, margin: 0 }}>Clicca su un giorno per vedere il dettaglio delle attività.</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div style={{maxHeight:420,overflowY:"auto",display:"flex",flexDirection:"column",gap:2}}>
          ...
          {fl.map((l,i)=><div key={l.id||i} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"10px 12px",background:i%2===0?"#FAFBFC":"white",borderRadius:8}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}><span style={{fontSize:12,fontWeight:700,padding:"2px 8px",borderRadius:100,background:PC[l.user]?.bg||"#F1F5F9",color:PC[l.user]?.fg||"#64748B"}}>{l.user}</span><span style={{fontSize:11,color:"#94A3B8"}}>{timeAgo(l.timestamp)}</span></div>
...
function GanttTimeline({tasks,vendors,currentUser}){
  ...
  const allD=sorted.flatMap(t=>[t.start,t.end]).filter(Boolean);
  const minD=allD.length>0?allD.reduce((a,b)=>a<b?a:b):new Date().toISOString().slice(0,10);
  const maxD=allD.length>0?allD.reduce((a,b)=>a>b?a:b):new Date().toISOString().slice(0,10);
  const sMs=new Date(minD).getTime()-86400000,eMs=new Date(maxD).getTime()+86400000*2;
  const tD=Math.max(Math.ceil((eMs-sMs)/86400000),7);
  function days(){const l=[];for(let i=0;i<tD;i++){const d=new Date(sMs+i*86400000);l.push({label:d.toLocaleDateString("it-IT",{day:"numeric",month:"short"}),isWe:d.getDay()===0||d.getDay()===6});}return l;}
  ...
  async function handleSave(){if(!form.name||!form.start||!form.end){alert("Compila nome, inizio e fine");return;}const t={...form,id:editId||gid()};try{await saveGanttTask(t);logActivity(currentUser,editId?"gantt_modificato":"gantt_creato",currentUser+(editId?" ha modificato":" ha creato")+" attività: "+form.name)}catch(e){alert("Errore: "+e.message)}setForm({name:"",vendor:"",start:"",end:"",color:GCOL[0],assignee:""});setEditId(null);}
  async function handleDel(id,name){if(!confirm("Eliminare?"))return;try{await removeGanttTask(id);logActivity(currentUser,"gantt_eliminato",currentUser+" ha eliminato: "+name)}catch(e){alert("Errore: "+e.message)}}
  const dl=days();
  return(
    <div style={{display:"flex",flexDirection:"column",gap:24}}>
      <div style={pnl}>
        <h3 style={{margin:"0 0 16px",fontSize:16,fontWeight:700}}>{editId?"✏️ Modifica":"📐 Nuova attività"}</h3>
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
          <input style={{...inp,flex:"2 1 180px"}} placeholder="Nome *" value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/>
      ...
      {sorted.length===0?<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"30vh",textAlign:"center"}}><div style={{fontSize:52,marginBottom:16}}>📐</div><h2 style={{margin:0,fontSize:22,fontWeight:700}}>Nessuna attività</h2></div>
      :<div style={{...pnl,padding:0,overflow:"hidden"}}>
        ...
        <div style={{overflowX:"auto"}}><div style={{minWidth:Math.max(tD*40,600)}}>
          ...
          {sorted.map(t=>{const b=bs(t);return<div key={t.id} style={{display:"flex",borderBottom:"1px solid #F8FAFC",minHeight:44}}>
            <div style={{width:220,flexShrink:0,padding:"8px 16px",display:"flex",alignItems:"center",gap:8,borderRight:"1px solid #E2E8F0"}}><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.name}</div><div style={{fontSize:11,color:"#94A3B8"}}>{t.vendor&&<span>{t.vendor}</span>}{t.vendor&&t.assignee&&" · "}{t.assignee&&<span style={{color:PC[t.assignee]?.fg||"#64748B"}}>{t.assignee}</span>}</div></div><button onClick={()=>{setForm({name:t.name,vendor:t.vendor||"",start:t.start,end:t.end,color:t.color||GCOL[0],assignee:t.assignee||""});setEditId(t.id)}} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,padding:2}}>✏️</button><button onClick={()=>handleDel(t.id,t.name)} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,padding:2}}>🗑️</button></div>
            <div style={{flex:1,position:"relative",display:"flex",alignItems:"center"}}>{dl.map((d,i)=>d.isWe&&<div key={i} style={{position:"absolute",left:(i/tD*100)+"%",width:(100/tD)+"%",top:0,bottom:0,background:"#F8FAFC"}}/>)}<div style={{position:"absolute",...b,height:28,borderRadius:6,background:t.color||"#3B82F6",opacity:.85,display:"flex",alignItems:"center",paddingLeft:8,overflow:"hidden"}}><span style={{fontSize:11,fontWeight:600,color:"white",whiteSpace:"nowrap",textShadow:"0 1px 2px rgba(0,0,0,0.3)"}}>{t.name}</span></div></div>
          </div>})}
        </div></div>
      </div>}
    </div>
...
function Scheduling({vendors,initVid,clearInit,addSlot,rmSlot,setResp,addAv,rmAv}){
  ...
  const vendor=vendors.find(v=>v.id===sel);const slots=fix(vendor?.slots);
    ...
    <div style={{display:"flex",flexDirection:"column",gap:24}}>
      ...
      {vendor&&<>
        <div style={pnl}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            {slots.map(s=>{
              const sLabel=STEPS.find(st=>st.key===s.step)?.label||s.step;const resp=s.responses||{};const stepParts=(vendor[s.step]?.participants?.length>0)?vendor[s.step].participants:[];const yC=stepParts.filter(p=>resp[p]==="yes").length,nC=stepParts.filter(p=>resp[p]==="no").length,pC=stepParts.filter(p=>!resp[p]).length;
              return<div key={s.id} style={{borderRadius:14,border:"1px solid #E2E8F0",overflow:"hidden"}}>
                ...
                <div style={{background:"white"}}>
                  ...
                  {stepParts.length===0&&<div style={{padding:20,textAlign:"center",color:"#94A3B8",fontSize:13}}>⚠️ Nessun partecipante assegnato. Vai in <strong>Vendor</strong> → modifica.</div>}
                  {stepParts.map((p,pi)=>{const val=resp[p];return<div key={p} style={{display:"grid",gridTemplateColumns:"120px 1fr 1fr 120px",alignItems:"center",padding:"12px 20px",borderBottom:pi<stepParts.length-1?"1px solid #F8FAFC":"none",background:val==="yes"?"#F0FDF4":val==="no"?"#FFF5F5":"white"}}>
                    ...
                    <div style={{textAlign:"center"}}>{!val&&<span style={{fontSize:12,color:"#F59E0B",fontWeight:700,animation:"pulse 2s infinite"}}>⏳</span>}{val==="yes"&&<span style={{fontSize:12,color:"#059669",fontWeight:700}}>✓ OK</span>}{val==="no"&&<span style={{fontSize:12,color:"#DC2626",fontWeight:700}}>✗ No</span>}</div>
                  </div>})}
                </div>
              </div>;
            })}
        ...
        <div style={pnl}>
          ...
          {(vendor.availability||[]).length===0&&<p style={{color:"#94A3B8",fontSize:14,textAlign:"center",padding:20}}>Nessuna disponibilità.</p>}
